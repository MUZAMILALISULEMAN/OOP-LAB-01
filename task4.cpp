...
while(1){
int isContinue=1;
cout<<"REGISTER FOR EVENT (1/2) OR MEMBERS IN REVERSE ORDER (3) ";
cin>>choice;
switch(choice){
	case 1:
	...
	cout<<endl<<"PARTICIPANTS IN (EVENT 1) : "<<endl<<endl;
	for(int i=event1_count-1;i>=0;i--){
		cout<<event1_participant[i]<<endl;
	}
	cout<<endl<<"PARTICIPANTS IN (EVENT 2) : "<<endl<<endl;
		for(int i=event2_count-1;i>=0;i--){
		cout<<event2_participant[i]<<endl;
	}
